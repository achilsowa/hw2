...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================        
        var, mu = torch.var_mean(inputs, dim=-1, keepdim=True, unbiased=False)
        x = (inputs-mu) / torch.sqrt(self.eps + var)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        self.bQ = nn.Parameter(torch.Tensor(dmodel))
        self.WK = nn.Parameter(torch.Tensor(dmodel, dmodel))
        self.bK = nn.Parameter(torch.Tensor(dmodel))
        self.WV = nn.Parameter(torch.Tensor(dmodel, dmodel))
        self.bV = nn.Parameter(torch.Tensor(dmodel))
        self.WO = nn.Parameter(torch.Tensor(dmodel, dmodel))
        self.bO = nn.Parameter(torch.Tensor(dmodel))
        
        self.reset_parameters()


    def reset_parameters(self):
        nn.init.ones_(self.WQ)
        nn.init.zeros_(self.bQ)
        nn.init.ones_(self.WK)
        nn.init.zeros_(self.bK)
        nn.init.ones_(self.WV)
        nn.init.zeros_(self.bV)
        nn.init.ones_(self.WO)
    ...
    def get_attention_weights(self, queries, keys):
        ...
        weights = queries @ keys.transpose(2, 3)
        
        mask = torch.tril(torch.ones(seq_length, seq_length))
        weights = weights*mask -1e4*(1-mask)
        return torch.softmax(weights/math.sqrt(self.head_size), dim=-1)
    ...
    def apply_attention(self, queries, keys, values):
        ...
        return self.merge_heads(weights @ values)
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        batch_size, seq_length, heads_x_dim = tensor.shape
        dim = heads_x_dim // self.num_heads
        return tensor.reshape((batch_size, seq_length, self.num_heads, dim)).transpose(1, 2)
    ...
    def merge_heads(self, tensor):
        ...
        return tensor.transpose(1, 2).reshape((batch_size, seq_length, num_heads * dim))
    ...
    def forward(self, hidden_states):
        ...
        # TODO: Write your code here
        # ==========================
        queries = self.split_heads(hidden_states @ self.WQ + self.bQ)
        keys = self.split_heads(hidden_states @ self.WK + self.bK)
        values = self.split_heads(hidden_states @ self.WV + self.bV)
        Y = self.apply_attention(queries, keys, values)
        return Y @ self.WO + self.bO
...
class MiniGPT1(nn.Module):
    ...
    def get_embeddings(self, inputs):
        ...
        positions = torch.zeros(inputs.shape).to(int) + torch.arange(seq_length)
        return self.embedding(inputs, positions)
    ...
    def forward(self, inputs):
        ...
        return torch.log_softmax(self.classifier(output), dim=-1)
    ...
    def loss(self, log_probas, targets, mask):
        ...
        # TODO: Write your code here
        # ==========================
        # Same as LSTM case loss
        
        # create one_hot representation of target, of shape `(batch_size, sequence_length, vocabulary_size)`
        vocab_size = log_probas.shape[2]
        targets_onehot = torch.LongTensor(targets.shape + (vocab_size, )).zero_()
        targets_onehot.scatter_(-1, targets.unsqueeze(-1).long(), 1)

        # for each seq in the batch, compute the length of the sentence. shape `(batch_size)`
